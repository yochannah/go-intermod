...
(defn ontology-query
  ...
  (let [service (get-service organism)
    ...
    (go (let [response (<! (http/post (str  (.-root service) "/service/query/results")
...
(defn resolve-ids
  ...
  (go (let [root (.-root (get-service source))
            response (<! (http/post (str  root "/service/ids")
                            {:with-credentials? false
                              :json-params (clj->js input)}))]
        (if-let [uid (-> response :body :uid)]
          (loop []
            (let [status-response (<! (http/get (str  root "/service/ids/" uid "/status")
                                                {:with-credentials? false}))]
              (if (= "SUCCESS" (:status (:body status-response)))
                (let [final-response (<! (http/get (str  root "/service/ids/" uid "/results")
                                                   {:with-credentials? false}))]
                  (http/delete (str  root "/service/ids/" uid)
                               {:with-credentials? false})
                  final-response)
                (do
                  (<! (timeout 1000))
...
(defn enrichment
  ...
  (go (let [response (<! (http/post (str  root "/service/list/enrichment")
   {:with-credentials? false
    :keywordize-keys? true
    :form-params (merge
     {:widget widget
